...
def main():
    ...
    for bi in range(nb):
        ...
        prompt = (
            ...
            "긍정 리뷰가 섞여 있을 수 있다). 이 리뷰들은 여러 다른 상품이 뒤섞여 있다 — "
            "**같은 배치 안에서 여러 번 반복돼야만 키워드로 인정하지 마라.** 다른 리뷰와 "
            "겹치지 않는 그 상품만의 단발성 이슈라도, 내용이 구체적인 불만·고장·개선요청이면 "
            "**1건짜리 키워드로도 반드시 뽑아라**(예: '산지 2일 만에 고장', '세탁 불가', "
            "'배터리 오래 안감' 같은 건 그 배치에 1건뿐이어도 절대 누락하지 마라). "
            "개수 제한 없이, 이 배치 안의 리뷰 중 구체적인 불만/개선요청이 담긴 것은 "
            "전부(1건짜리 포함) 키워드로 추출하라. "
            ...
            "단순히 무난하거나 미온적인 칭찬뿐이고 구체적인 불만·요청이 전혀 없는 리뷰(예: '아직 "
            "사용전이에요', '잘 받았습니다', '좋을 것 같아요' 같은 막연한 기대/인사말)만 "
            "클러스터링 대상에서 제외하라.\n"
