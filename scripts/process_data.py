...
COMPLAINT_PATTERNS: List[tuple] = [
    # 2026-08: "소리.*크"는 "크다" 활용형인 "커요/커서/큰것/컸다"를 문자열 수준에서
    # 놓침("크"+어→"커" 모음축약이라 "크" 글자 자체가 안 남음) — 5개월 실측 검증(67→78건).
    ("소음",        r"소음이?|소리.*(크|커|큰|컸)|시끄럽|웅.*소리|귀.*울림|머리.*울",        "소음/시끄러움"),
    # 2026-08: "환불.*안"류가 "안 되다"만 잡고 "~지 않다"(안↔않, 흔한 부정 표현 두 방식)를
    # 놓쳐서 보강 — 단, 처음엔 bare "안|않"으로 넓혔다가 "어렵지 않아요"(=쉽다, 칭찬)처럼
    # 부정 대상 형용사 전부에 오탐이 나서(실측), "되다/돼다/됨"에 붙을 때로 좁힘.
    # 2026-08 추가 보강: "반품안된다네요/반품이 안된다고"의 "안된"(안+되+ㄴ, "되다"의 관형형
    # 활용)이 기존 "안\s?(되|돼|됨)"엔 없어서 누락 — "고장" 패턴엔 이미 "된"이 있었는데
    # 환불반품/AS문제엔 빠져있던 것을 5개월 실측(2건, 오탐 0)으로 확인 후 동일하게 확장.
    # 단 "안된다면/안된다는게 아니라"류(가정법·부정의 부정)는 실측상 드물지만 제외 게이트.
    ("환불반품",    r"환불.*(안\s?(되|돼|됨|된(?!다면|다는게\s?아니|다는건\s?아니))|되지\s?않)|반품.*(안\s?(되|돼|됨|된(?!다면|다는게\s?아니|다는건\s?아니))|되지\s?않)|반품.*힘|환불.*못|환불.*불가|반품.*불가|개봉.*반품",  "환불·반품 불가"),
    # "안시원" 접두부정("시원하지 않다"만 있고 "안 시원해요"류 접두형이 없었음) — 5개월
    # 실측 9건, 오탐 0. "성능이 별로/안좋/나쁘/아쉬/부족" 부정구성 추가 — 실측 3건, 오탐 0
    # (bare "성능"은 중립 서술에도 쓰여 부정구성으로 좁힘).
    ("효과없음",    r"효과.*없|시원하지.*않|안\s?시원|별로|아무.*효과|효용.*못|모르겠|느껴지지.*않|느낌이.*없|약해|강도.*약|성능.{0,10}(별로|안\s?좋|나쁘|아쉬|부족|떨어|그닥|그저)",  "효과 미흡"),
    ("AS문제",     r"AS.*(안\s?(되|돼|됨|된)|되지\s?않)|as.*(안\s?(되|돼|됨|된)|되지\s?않)|고객.*과실|상담원.*연결.*(안\s?되|되지\s?않)|전화.*연결.*(안\s?되|되지\s?않)|전화.*수신.*거절|ai.*상담만",  "AS/고객서비스 문제"),
    # 2026-08: "고장났"라는 정확한 활용형만 잡고 있어서 "고장이 나서/고장남/고장인것같다/
    # 작동이 되지 않습니다"(안↔않) 등 압도적으로 많은 나머지 활용형을 다 놓치고 있었음 —
    # 5개월 실측 30여건 추가 포착(기존 COMPLAINT_PATTERNS 중 최대 규모 보강). 단 bare "고장"
    # 하나만으로는 "고장없이 오래 썼으면"(칭찬), "안고장나고"(칭찬), "전에 쓰던 게 고장나서
    # 이걸로 바꿨어요"(타제품 얘기) 같은 오탐이 실측으로 확인돼, "고장없이/안 나고/날 우려"
    # 앞뒤로는 제외. "타제품이 고장나서 바꿨다"류 잔여 오탐은 reverify_suspect.py의 AI
    # ①관련성 게이트(다른 제품 얘기는 '무관' 처리)가 걸러내는 몫으로 남김.
    # 2026-08 추가 보강: "작동이 안된답니다"의 "안된"(안+되+ㄴ다, "되다"의 또다른 활용형)이
    # 기존 "안\s?(되|돼|됨)"엔 없어서 누락 — 실측 오탐 0건으로 안전 확인 후 추가.
    ("고장",        r"(?<!안)고장(?!\s?(이|은|도)?\s?(없|나지\s?않|날\s?우려|안\s?나))|작동.{0,10}(안\s?(되|돼|됨|된)|되지\s?않|돼지\s?않)|전원.{0,10}(안\s?(되|돼|됨|들어오)|들어오지\s?않)",   "고장/작동불량"),
    # "파손없이/파손되지 않고"(포장이 안전하게 왔다는 칭찬)가 걸리던 것 실측으로 확인해 제외.
    # ("깨지다"=잠이 깨다 동음이의어 충돌처럼 아주 드문 잔여 오탐은 reverify_suspect.py 몫으로 남김.)
    # "부러질"(미래/추측형, "부러지"엔 없던 활용형) 추가.
    ("파손",        r"(파손|부러지|부러질|부서지|깨지)(?!\s?(없이|되지\s?않|지\s?않))",   "부품/제품 파손"),
    # 2026-08 신규: '강도'는 그동안 IMPROVEMENT_PATTERNS(개선요청)에만 있어, "~약하네요/
    # ~약해서 아쉬워요"처럼 요청이 아니라 그냥 불만으로 끝나는 어조가 통째로 새고 있었음
    # — 실측 7건. 정밀도는 낮은 편(과거 "처음엔 약했는데 이제 좋아요"류 사용팁 설명과
    # 헷갈림, 11건 중 4건 오탐) — reverify_suspect.py AI 게이트가 걸러내는 몫으로 남김.
    ("강도약함불만", r"(압력|두드림|롤러|자극|힘|파워).{0,8}(약하|약해)|힘이\s?약",  "강도 약함(불만)"),
    # 2026-08 신규: 강도약함불만의 반대방향(너무 강함/셈) — "쎈/쌘"(쎄다·쌔다의 관형형),
    # "커서/크게 느껴/엄청 커" 등 "크다" 계열도 강도 과함 표현으로 씀. "너무 강하지 않고"/
    # "쌔지않음"처럼 부정(否定)된 경우는 오히려 칭찬·약함쪽이라 부정 후방탐색으로 제외
    # (5개월 실측 9건, 오탐 0).
    ("강도과함",    r"(강도|세기|힘|압력|자극|진동).{0,15}(너무\s?(세|강)|쎄|쌔|씨네|쎈|쌘|커서|크게\s?느껴|엄청\s?커)(?!.{0,4}지\s?않)",  "강도/진동 과함(불만)"),
    ("과장광고",    r"광고.*다르|과대광고|속아|속은|속았|광고와|과장",              "과장광고"),
    ("높이불편",    r"높이.*높|높아서.*불편|너무.*높|높이.*불편",                   "높이 불편"),
    ("진동두통",    r"진동.*골|골이.*울|두통|머리.*울려|골이.*울려",               "진동 두통"),
    # "AI로만"만 쓰고 "상담"이란 단어를 안 쓰는 리뷰(예: "AI로만 운영", "AI로만 대화")를
    # 놓치던 것 보강 — 5개월 실측 3건.
    ("AI상담",     r"AI.*상담|ai.*상담|상담원.*연결.*안되|전화.*안됨|AI로만|ai로만|AI밖에|ai밖에",  "AI상담·전화 불가"),
    ("가격대비",    r"가격.*비싸|돈.*아까|돈.*비해|비싸고|가성비.*나쁜",           "가격 대비 효과"),
    # "충전.*안되"만으로는 "충전 지속시간이 짧다"류 불만을 못 잡아 보강.
    ("충전불편",    r"전원코드.*연결|코드.*연결.*사용|무선.*아니|충전.*안되|배터리.*없|충전.{0,6}(짧|금방\s?닳|금방\s?떨어)",  "유선/충전 불편"),
    # 2026-08: 마사지 후 오히려 통증이 심해지는 '역효과' 사례. 초기 버전(오히려/더아프
    # 단독)은 "세면 오히려 안좋을듯"(가정법 칭찬) 오탐이 많아, 실제 사용 맥락
    # (사용/쓰고/자고/하고 + 후)과 인접할 때만 잡히게 좁힘 — 실측 검증.
    ("역효과",      r"(사용|쓰고|자고|하고).{0,15}(더\s?아프|더\s?아파)|후.{0,10}(더\s?아프|더\s?아파)",  "역효과(사용 후 악화)"),
    # ── 2026-08 추가: 미포착 부정리뷰 125건 실측 분석 기반 신규 패턴 ──
    # "칙칙이"는 "찍찍이"(벨크로)의 흔한 오타 표기 — 5개월 실측에서 발견해 추가.
    ("고정불량",    r"벨크로|찍찍이|칙칙이|접착력|고정.{0,4}(안|풀|떨어)|부착.{0,4}(안|떨어|반복)",  "벨크로/고정 불량"),
    # 2026-08: 끈/줄/밴드 길이 부족 — "고정불량"(탈부착 자체 불량)과는 별개로,
    # "길이가 짧아 착용이 안 된다"는 계열이 5개월 실측 7건으로 따로 존재.
    ("끈줄밴드짧음", r"(끈|줄|밴드).{0,10}(짧|짧아|짧은)",  "끈/줄/밴드 길이 부족"),
    ("사이즈안맞음", r"사이즈.{0,6}(작|안\s?맞|여성용)|팔.{0,10}굵.{0,10}(비추|안\s?맞|불편)|엉덩이.{0,6}크|너무\s?작아요|좀\s?작습니다|좀\s?작아요",  "사이즈 안 맞음"),
    # "뜯어졌"("뜯어지"+었, 활용형 누락), "밖음질"("박음질" 흔한 오타 표기) 추가.
    ("재질마감불량", r"보풀|찢어지|뜯어지|뜯어졌|흰가루|박음질|밖음질|피부.{0,4}까|마감.{0,4}불량",  "재질/마감 불량"),
    # 2026-08: discover_keywords.py(AI) 후보 재검토로 추가 발견 — 5개월 실측 검증.
    ("저온화상",    r"화상|데었|데임",  "저온화상 안전 이슈"),
    ("부속품누락",  r"(설명서|케이블|구성품|부속품|어댑터).{0,10}(누락|빠짐|안\s?옴)|누락.{0,10}(설명서|케이블|구성품|부속품)",  "부속품/구성품 누락"),
    ("좌우구분없음", r"좌우.{0,6}구분|왼발.{0,6}오른발|양쪽.{0,6}구분",  "좌우 구분 없음"),
    ("에어백이탈",  r"에어백.{0,8}(튀어나|빠지|이탈)|쿠션.{0,8}(빠지|이탈)|패드.{0,8}(빠지|이탈)",  "에어백/쿠션 이탈"),
    ("무선유선혼동", r"무선인줄|무선인가.{0,4}유선|유선이구요|유선이었|충전.{0,6}시원치|충전.{0,6}뻑뻑|저속충전기",  "무선/유선 혼동"),
    ("가격프로모션불공정", r"이벤트.{0,6}(안|않).{0,4}(적용|되)|프로모션.{0,4}(다르|안)|할인.{0,4}링크|싸게\s?파",  "프로모션/가격 불공정"),
    ("고장재발",    r"세\s?번째.{0,6}고장|또\s?고장|고장.{0,6}재구매",             "잦은 고장(재발)"),
    # 2026-08 신규: "고장"이란 단어 없이 "불량"만 쓰는 리뷰가 상당수 있었음(실측 10건,
    # 오탐 0) — 단 "자세 불량"(전혀 다른 뜻)과 "불량은 아니네요"(부정=칭찬)는 제외.
    ("제품불량",    r"(?<!자세\s)(?<!자세)불량(?!\s?(은|이)?\s?아니)",   "제품 불량"),
    # 2026-08 신규: "고장"과 별개로 "작동이 멈춘다/자꾸 꺼진다" 표현 — 활용형(멈추/멈춘/
    # 멈춤/멈춥) 다 포함. 5개월 실측 5건, 오탐 0.
    ("작동중단",    r"(작동|기기).{0,8}(멈추|멈춘|멈춤|멈춥)|자꾸\s?꺼|갑자기\s?꺼",  "작동 중단/멈춤"),
]
...
# 2026-08: 홑 "필요"/"원해"는 "필요했던 제품"(칭찬)처럼 모호하게도 쓰여 개선 풀을
# 전 감성으로 넓히며 오포착 위험이 커진 만큼 제거 — 명시적 요망형만 남김.
# "원합니"는 "시원합니다"(칭찬)의 "시"+"원합니다"와 우연히 겹쳐 오탐 발생 —
# 실측 검증(2026-07, 전 감성 풀 확장 후 스팟체크)으로 발견, 부정 후방탐색으로 차단.
# "불가능/불가"는 5개월 실측(예: "커버 세탁이 불가능해서 불편") 추가 — "안되"류와 같은 결핍 신호.
# 2026-08 추가: "었으면"(모음조화 어→었)만 있고 "았으면"(모음조화 아→았, 예: "나았으면
# 좋겠네요", "강도가 높았으면 좋겠어요")이 빠져있었음 — 전체 리뷰 실측 13건, 오탐 0.
_IMP_SIGNAL = (
    r"약하|약함|약해|아쉽|아쉬|부족|미흡|없어서|없네|없으니|없다|안\s?되|안\s?돼|안\s?됨|불편|곤란|힘들|불만족|"
    r"불가능|불가한|"
    r"었으면|였으면|았으면|했으면|면\s?좋|되면\s?좋|해주|해줬으면|있으면\s?좋|있었으면|바랍|(?<!시)원합니"
)
...
IMPROVEMENT_PATTERNS: List[tuple] = [
    ...
    # 2026-08 추가: 미포착 부정리뷰 실측 분석 기반
    ("고정개선요청", _imp(r"벨크로|찍찍이|접착|끈|줄|밴드"),     "고정/부착 개선 요청"),
    ("커버세탁",   _imp(r"커버|세탁|빨래|세척|물세척|분리형|벗겨"), "커버/세탁 개선"),
    ("냄새개선",   _imp(r"냄새|고무냄새|플라스틱냄새|쿰쿰"),        "냄새 개선"),
]
...
def extract_keywords_basic(df: pd.DataFrame, top_n: int = 30) -> dict:
    ...
    imp_pool = df.reset_index(drop=True)
...
class QuotaExhaustedError(RuntimeError):
    """Claude CLI 세션 한도 소진 — quota_retry.py 가 리셋 후 재시도하도록 상위에서 exit(3) 처리."""
...
def reclassify_keyword_full(
    ...
    if progress_path is not None and progress_path.is_file():
        try:
            done = json.loads(progress_path.read_text(encoding="utf-8")).get("done", {})
            if done:
                logger.info("  [RESUME] 전체 재분류 이어받기 — 완료 %d개 키워드는 재호출 없이 적용", len(done))
        except Exception:
            done = {}
    ...
    def _save_progress() -> None:
        ...
        try:
            ...
            progress_path.write_text(json.dumps({"done": done}, ensure_ascii=False), encoding="utf-8")
        except Exception as exc:  # noqa: BLE001
            logger.warning("진행 마커 저장 실패(계속 진행): %s", exc)
    ...
    for key, polarity in polarity_map.items():
        for item in bi.get(key, []):
            ...
            if word_key in done:
                # 이전 실행(한도 소진 전)에 이미 끝난 키워드 — AI 재호출 없이 저장분 그대로 적용
                saved = done[word_key]
                item["all_review_ids"] = saved["all_review_ids"]
                item["count"] = saved["count"]
                item["ai_reclassified"] = True
                item["review_samples"] = saved["review_samples"]
                item["by_product"] = saved["by_product"]
                continue
    ...
    if progress_path is not None and progress_path.is_file():
        try: progress_path.unlink()  # 이번 실행 정상 완료 — 다음 CSV 업로드 때 stale 진행분이 남지 않도록 제거
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    # 출력 경로를 미리 계산(검증 포함) — reclassify_keyword_full 의 quota resume 마커가
    # AI 단계(6-b)에서부터 필요하므로 8단계보다 앞서 확정해둔다.
    docs_root = (PROJECT_ROOT / "docs" / "data").resolve()
    try:
        out_dir = resolve_safe_output_dir(docs_root, brand, month)
    except ValueError as exc:
        logger.error("출력 경로 오류: %s", exc)
        sys.exit(1)
