...
COMPLAINT_PATTERNS: List[tuple] = [
    ...
    ("환불반품",    r"환불.*(안\s?(되|돼|됨)|되지\s?않)|반품.*(안\s?(되|돼|됨)|되지\s?않)|반품.*힘|환불.*못|환불.*불가|반품.*불가|개봉.*반품",  "환불·반품 불가"),
    ("효과없음",    r"효과.*없|시원하지.*않|별로|아무.*효과|효용.*못|모르겠|느껴지지.*않|느낌이.*없|약해|강도.*약",  "효과 미흡"),
    ("AS문제",     r"AS.*(안\s?(되|돼|됨)|되지\s?않)|as.*(안\s?(되|돼|됨)|되지\s?않)|고객.*과실|상담원.*연결.*(안\s?되|되지\s?않)|전화.*연결.*(안\s?되|되지\s?않)|전화.*수신.*거절|ai.*상담만",  "AS/고객서비스 문제"),
    ...
    ("고장",        r"(?<!안)고장(?!\s?(이|은|도)?\s?(없|나지\s?않|날\s?우려|안\s?나))|작동.{0,10}(안\s?(되|돼|됨)|되지\s?않|돼지\s?않)|전원.{0,10}(안\s?(되|돼|됨|들어오)|들어오지\s?않)",   "고장/작동불량"),
    # "파손없이/파손되지 않고"(포장이 안전하게 왔다는 칭찬)가 걸리던 것 실측으로 확인해 제외.
    # ("깨지다"=잠이 깨다 동음이의어 충돌처럼 아주 드문 잔여 오탐은 reverify_suspect.py 몫으로 남김.)
    ("파손",        r"(파손|부러지|부서지|깨지)(?!\s?(없이|되지\s?않|지\s?않))",   "부품/제품 파손"),
    ...
    ("재질마감불량", r"보풀|찢어지|뜯어지|흰가루|박음질|피부.{0,4}까|마감.{0,4}불량",  "재질/마감 불량"),
...
_IMP_SIGNAL = (
    ...
    r"불가능|불가한|"
    r"었으면|였으면|했으면|면\s?좋|되면\s?좋|해주|해줬으면|있으면\s?좋|있었으면|바랍|(?<!시)원합니"
)
