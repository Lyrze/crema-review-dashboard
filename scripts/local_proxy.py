...
한 개의 로컬 포트(기본 8799)로 두 가지를 처리한다:
  1. /api/*, / (health)  → Claude Code CLI 구독 인증으로 처리 (GPU 불필요)
  2. /gh/upload|list|load → GitHub Contents API로 Taxonomy 분류값 커밋/목록/조회

대시보드(docs/index.html)는 Ollama API 모양(GET / · GET /api/tags · POST /api/generate)
으로 말을 걸므로, 프록시가 같은 모양으로 응답하돼 내부적으로 Claude CLI를 호출한다
(브라우저 쪽 코드는 수정 없이 그대로 동작한다).
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if u.path.startswith("/api/generate"):
