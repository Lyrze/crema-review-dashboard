...
def main():
    ...
    eprint(f"  PVOC 의도 분류: {args.brand}/{args.month} · 리뷰 {len(reviews)}건 · 모델 {args.model}")

    analyzer = OllamaAnalyzer(model=args.model, base_url=args.base_url)
    if not analyzer.health_check():
        eprint("[ERROR] Ollama 응답 없음 — 중단"); sys.exit(1)
    ...
    out_path.write_text(json.dumps({
        "topics": result_topics, "model": args.model,
