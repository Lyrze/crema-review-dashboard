...
:: 결과 파싱
set "BRAND=" & set "MONTH=" & set "CSV=" & set "PREV_FLAG=" & set "AI_FLAG=" & set "RECLASS_FLAG=" & set "ANON_OUT="
...
:: 처리 결과
python scripts\show_result.py "!BRAND!" "!MONTH!" 2>/dev/null

:: 익명화 CSV
echo.
echo  익명화 CSV 생성 중...
python scriptsnonymize_csv.py --input "!CSV!" --output "!ANON_OUT!"
if errorlevel 1 ( echo  [WARNING] 익명화 실패. 계속 진행합니다. )

:: GitHub 푸시
echo.
echo  GitHub 푸시 중...
git add docs\data\ datanonymized\ .gitignore scriptsgit commit -m "data: !BRAND! !MONTH! review update"
